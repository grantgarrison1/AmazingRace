...
class Race(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null=True, blank=True, related_name='created_races')
...
class Zone(models.Model):
    ...
    def __str__(self):
        return f"{self.race.name} - Zone {self.number}"
...
class Question(models.Model):
    ...
    def __str__(self):
        return f"{self.zone.race.name} - Z{self.zone.number} - {self.question_text[:50]}"
